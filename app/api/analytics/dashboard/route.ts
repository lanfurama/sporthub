  ...
  try {
    ...
    const [
      ...
      prisma.booking.aggregate({
        where: { bookingDate: { gte: today, lt: tomorrow }, status: BookingStatus.confirmed },
      ...
      prisma.booking.groupBy({
        by: ['bookingDate'],
        where: {
          bookingDate: { gte: sevenDaysAgo, lt: tomorrow },
          status: { notIn: [BookingStatus.cancelled, BookingStatus.rejected] },
        },
        _count: { _all: true },
    ...
    const memberMap = memberCounts.reduce((acc: Record<string, number>, m) => {
      const count =
        typeof m._count === 'object' && m._count !== null && '_all' in m._count
    ...
    const groupedByDate = dailyBookingsGrouped.reduce((acc: Record<string, number>, d) => {
      ...
      const count =
        typeof d._count === 'object' && d._count !== null && '_all' in d._count
    ...
    const dailyBookings = Array.from({ length: 7 }, (_, i) => {
      ...
      return { date: key, bookings: groupedByDate[key] ?? 0 };
    });
